...
import { Play, Volume2, VolumeX, BarChart2, Sparkles, ArrowUpRight } from 'lucide-react';
...
function Slide3_Music({ data }: { data: Slide3Data }) {
  ...
  const thumb = track.youtubeId ? ytThumb(track.youtubeId, 'hq') : '';
  const startSec = 0;
  ...
  useEffect(() => {
    ...
    (window as any).onYouTubeIframeAPIReady = () => {
      createPlayer(tracks[0].youtubeId, 0);
    };
  ...
  useEffect(() => {
    if (player && player.loadVideoById && track.youtubeId) {
      player.loadVideoById({ videoId: track.youtubeId, startSeconds: startSec });
        ...
        <div className="relative z-10 w-full h-full flex flex-col justify-center px-6 py-6 md:p-16 lg:p-24 overflow-y-auto custom-scrollbar">
          ...
          <div className="flex flex-col items-start gap-2 md:gap-6 mb-4 mt-2 shrink-0 w-full max-w-sm md:max-w-md">
            <motion.button
              ...
              className="w-24 h-24 sm:w-28 sm:h-28 md:w-64 md:h-64 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.8)] border-[3px] md:border-[6px] border-[#050505] overflow-hidden shrink-0 relative cursor-pointer group outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              animate={{ rotate: isPlaying ? 360 : 0 }}
              transition={{ duration: 15, repeat: isPlaying ? Infinity : 0, ease: "linear" }}
            >
              {thumb && <img src={thumb} className="w-full h-full object-cover scale-125" />}
              <div className="absolute inset-0 flex items-center justify-center bg-black/30 group-hover:bg-black/10 transition-colors">
                {!isPlaying ? (
                  <div className="w-8 h-8 md:w-16 md:h-16 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center border border-white/20">
                    <Play className="w-3 h-3 md:w-6 md:h-6 text-white ml-0.5 md:ml-1 fill-current" />
                  </div>
                ) : (
                  <div className="w-8 h-8 md:w-16 md:h-16 bg-black/30 backdrop-blur-sm border border-transparent hover:border-white/20 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                    <div className="w-3 h-3 md:w-5 md:h-5 border-l-2 md:border-l-[3px] border-r-2 md:border-r-[3px] border-white gap-1 md:gap-1.5 flex h-4 md:h-5"></div>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col gap-3 md:gap-4 shrink-0 items-start w-full max-w-sm">
            <div className="flex items-center justify-start gap-2 sm:gap-3 w-full">
              <motion.a
                ...
                className={`flex items-center justify-center gap-2 bg-[#FF0000] text-white py-2.5 px-4 sm:px-5 md:py-4 md:px-8 rounded-full transition-all font-bold text-[10px] sm:text-xs md:text-sm tracking-wide shadow-xl shrink-0 group border-none outline-none cursor-pointer whitespace-nowrap ${!data.playlistUrl ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''}`}
              >
              ...
              <div className="w-12 h-12 md:w-16 md:h-16 rounded-lg md:rounded-xl overflow-hidden relative shrink-0 bg-black">
                ...
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                  {activeIdx === i
                    ...
                    : <Play className="w-4 h-4 md:w-6 md:h-6 text-white opacity-50 group-hover:opacity-100 transition-opacity fill-current" />}
                </div>
              </div>
