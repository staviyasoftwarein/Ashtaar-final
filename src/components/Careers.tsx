...
export default function Careers() {
  ...
  // Warm browser cache for every active role's hover image so hover swaps are instant.
  useEffect(() => {
    const urls = rolesData.map((r) => resolveCareerImageUrl(r.imagePath)).filter(Boolean);
    ...
    return () => { handles.forEach((h) => { h.src = ''; }); };
