...
import { DEFAULT_TEAM, resolveTeamImageUrl, type TeamConfig } from '../lib/team';

export default function Team() {
  const containerRef = useRef<HTMLDivElement>(null);
  const { value: cfg } = useSetting<TeamConfig>('team', DEFAULT_TEAM);
  const team = cfg.members.length > 0 ? cfg.members : DEFAULT_TEAM.members;
  ...
  return (
    <div id="team" ref={containerRef} className="min-h-screen py-32 px-6 md:px-12 flex flex-col justify-center relative z-20 overflow-hidden bg-transparent">
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[url('/bts4.jpg')] bg-cover bg-center opacity-10 mix-blend-screen grayscale"></div>
      ...
      <div className="max-w-5xl mx-auto w-full perspective-[1200px]">
        ...
        >
          <div className="text-center mb-16">
            <h3 className="text-[#D4AF37] uppercase tracking-[0.3em] text-xs md:text-sm mb-6 font-sans font-bold">{cfg.eyebrow || DEFAULT_TEAM.eyebrow}</h3>
            <h2 className="font-serif text-4xl md:text-6xl lg:text-7xl text-white">{cfg.title || DEFAULT_TEAM.title}</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 max-w-4xl mx-auto">
            {team.map((member, i) => {
              const url = resolveTeamImageUrl(member.imagePath);
              return (
                <div
                  key={member.id || i}
                ...
                >
                  <div className="aspect-[3/4] overflow-hidden bg-gray-200 mb-6 relative">
                    {url ? (
                      <img src={url} alt={member.name} className="w-full h-full object-cover grayscale-0 group-hover:grayscale-[60%] transition-all duration-700 scale-100 group-hover:scale-105" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-gray-400 font-mono text-[10px] uppercase tracking-[0.2em]">
                        No photo
                      </div>
                    )}
